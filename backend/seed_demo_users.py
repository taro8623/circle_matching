...
DEMO_USERS = [
    DemoUserSpec(
        name="葵",
        email="demo.user.01@example.com",
        legacy_email="demo.roots.01@example.com",
    ...
    DemoUserSpec(
        name="蓮",
        email="demo.user.02@example.com",
        legacy_email="demo.roots.02@example.com",
    ...
    DemoUserSpec(
        name="芽衣",
        email="demo.user.03@example.com",
        legacy_email="demo.roots.03@example.com",
    ...
    DemoUserSpec(
        name="駿",
        email="demo.user.04@example.com",
        legacy_email="demo.roots.04@example.com",
    ...
    DemoUserSpec(
        name="奈々",
        email="demo.user.05@example.com",
        legacy_email="demo.roots.05@example.com",
    ...
    DemoUserSpec(
        name="陽",
        email="demo.user.06@example.com",
        legacy_email="demo.roots.06@example.com",
    ...
    DemoUserSpec(
        name="悠樹",
        email="demo.user.07@example.com",
        legacy_email="demo.roots.07@example.com",
    ...
    DemoUserSpec(
        name="康太",
        email="demo.user.08@example.com",
        legacy_email="demo.roots.08@example.com",
    ...
    DemoUserSpec(
        name="里奈",
        email="demo.user.09@example.com",
        legacy_email="demo.roots.09@example.com",
    ...
    DemoUserSpec(
        ...
        legacy_email="demo.roots.10@example.com",
...
OWNER_SPEC = DemoUserSpec(
    ...
    legacy_email="demo.roots.owner@example.com",
    bio="公開デモ運営アカウントです。ライブ管理や権限設定の挙動確認に使えます。",
    parts=[("Gt", None)],
...
def upsert_user(db, spec: DemoUserSpec, password: str) -> tuple[User, bool]:
    ...
    if user is None and spec.legacy_email:
        user = db.query(User).filter(User.email == spec.legacy_email).first()
    created = False
    if user is None:
        user = User(
            name=spec.name,
...
def leave_legacy_demo_circle(db, user: User, target_circle: Circle, legacy_circle_name: str | None) -> None:
    if not legacy_circle_name or legacy_circle_name == target_circle.name:
        return

    legacy_circle = db.query(Circle).filter(Circle.name == legacy_circle_name).first()
    if legacy_circle is None:
        return

    membership = (
        db.query(CircleMember)
        .filter(
            CircleMember.circle_id == legacy_circle.id,
            CircleMember.user_id == user.id,
...
def main() -> None:
    password = os.environ.get("DEMO_USER_PASSWORD", "CircleDemo2026!")
    circle_name = os.environ.get("DEMO_CIRCLE_NAME", "Circle Matching Demo")
    join_password = os.environ.get("DEMO_CIRCLE_JOIN_PASSWORD", "circle-demo")
    legacy_circle_name = os.environ.get("LEGACY_DEMO_CIRCLE_NAME", "roots")
    db = SessionLocal()
    ...
    try:
        ...
        revived_count = 0
        leave_legacy_demo_circle(db, owner, circle, legacy_circle_name)
        ...
        for spec in DEMO_USERS:
            ...
            if created:
                created_count += 1
            leave_legacy_demo_circle(db, user, circle, legacy_circle_name)
